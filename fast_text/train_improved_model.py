...
1. Extracting text from PDF documents in both argilla_data_49 and data_documents
2. Using the labeled Excel data as high-quality training examples
3. Implementing better text preprocessing for Italian tax documents
4. Using cross-validation to evaluate and tune the model
5. Optimizing hyperparameters for better classification
"""
...
def clean_text(text: str) -> str:
    """
    Enhanced text cleaning function for Italian tax documents.
    ...
    # Check if any tax terms are in the text before cleaning
    has_tax_terms = any(term in text for term in TAX_TERMS)
    
    # Replace common Italian abbreviations with full forms
    replacements = {
        "art.": "articolo",
        "p.iva": "partita iva",
        "c.f.": "codice fiscale",
    ...
    # IMPORTANT: Preserve key terms in the text
    # Ensure "iva" is preserved and not expanded/replaced in processing
    if "iva" in text.lower().split():
        text = text + " imposta_valore_aggiunto tasse fiscale"
    
    # Add tax-related terms for positive examples to strengthen the signal
    if has_tax_terms:
        tax_boosting_terms = "tasse fiscale iva dichiarazione"
        text = text + " " + tax_boosting_terms
...
def generate_negative_examples(positive_examples: List[Tuple[str, str]], 
                               count: int = 1000) -> List[Tuple[str, str]]:
    """
    ...
    """
    # Common Italian phrases unrelated to taxes
    common_topics = [
    ...
    for _ in range(min(count // 2, len(common_topics))):
        topic = random.choice(common_topics)
        negative_examples.append((topic, 'Other'))
    
    # Generate more diverse negative examples
    words_collection = []
    for text, _ in positive_examples:
        words_collection.extend(text.split())
    
    # Count word frequencies
    word_counter = Counter(words_collection)
    common_words = [word for word, count in word_counter.most_common(100) 
                   if word not in ITALIAN_STOPWORDS and len(word) > 3 
                   and word not in TAX_TERMS]  # Exclude tax terms from negative examples
    ...
    for _ in range(count - len(negative_examples)):
        ...
        if len(common_words) > 0:  # Only proceed if we have common words
            sentence_words = random.sample(common_words, min(num_words, len(common_words)))
            random.shuffle(sentence_words)  # Randomize word order
...
def train_fasttext_model(training_data: List[Tuple[str, str]], 
                         output_path: str,
    ...
    try:
        ...
        # Train the model
        logger.info(f"Training FastText model with {len(training_data)} examples...")
        ...
        logger.info(f"FastText model trained and saved to {output_path}")
    ...
    try:
        ...
        for params in combinations:
            ...
            if success:
                ...
                logger.info(f"Parameters {params} achieved accuracy: {accuracy:.4f}")
...
def main():
    ...
    logger.info(f"Split data: {len(train_data)} training, {len(val_data)} validation, {len(test_data)} test examples")
